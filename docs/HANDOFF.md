...
5. ⚠️ **Pendência nova (2026-07-03): não existe UI de admin para editar o tema (`theme_settings`).** A tabela (`primary_color`, `secondary_color`, `accent_color`, `background_color`, `text_primary_color`, `text_secondary_color`) e o mecanismo de renderização (`lib/content/theme.ts` → `themeToCssVars` → injetado como `<style>:root{...}</style>` em `app/(portal)/layout.tsx`) existem e funcionam (validado via UPDATE SQL direto — reflete no HTML na hora, sem cache), mas `app/admin/(painel)/configuracoes/form.tsx` só tem as abas Geral/Contato/Redes/SEO — nenhum campo de cor foi conectado a essa tabela. Nem o plano original do admin (`docs/superpowers/plans/2026-07-01-portal-institucional-admin-plan.md`) nem o do portal público (`docs/superpowers/plans/2026-07-02-portal-publico-plan.md`) têm uma task explícita de "aba Aparência" — parece um gap entre os dois planos (o portal público assumiu que a edição já existiria). Falta decidir com o Carlos: criar uma nova task (aba "Aparência" em Configurações com color pickers salvando em `theme_settings`) ou manter o tema fixo via SQL/migration.
