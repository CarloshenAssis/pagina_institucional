...
| 10 | **SEO completo** | Pedido "configurar o SEO" — auditoria da produção achou: (a) **bug real** — `whatsapp_url` tinha um número puro (`"(12) 99184-1312"`) em vez de link, os botões de WhatsApp (rodapé + `/contato`) ficavam quebrados; novo `lib/content/whatsapp.ts` normaliza qualquer formato pra um link `wa.me` válido, sem precisar reeditar o dado salvo; (b) `metadataBase` adicionado ao layout do portal; (c) novo `personJsonLd` (schema.org `Person`) renderizado na Home com nome/descrição/foto do Sobre + redes sociais — ajuda o Google a entender o assunto do site; (d) novo campo "Código de verificação — Google Search Console" na aba SEO (migration `0010_seo_verification.sql` — `saveSettings` faz UPDATE por chave, não upsert, então uma chave nova precisa existir na tabela antes de poder ser salva pelo admin). **Pendência do Carlos:** a imagem Open Graph (1200×630) segue vazia — precisa escolher/enviar uma foto na aba SEO; não dá pra gerar via código | **SIM — `0010_seo_verification.sql`** | `916f37d` |
...
### ✅ Repositório-base de revenda — RE-SINCRONIZADO (2026-07-04)
O `Portal_Institucional_Base` (`github.com/CarloshenAssis/Portal_Institucional_Base`) foi atualizado com todos os itens 1–9 desta tabela, commit `9906f49`. Processo: recopiar o código de `main` para o workspace do repo base (`tar` com exclusões de `.git`/`node_modules`/`.next`/`docs/HANDOFF.md`/`docs/superpowers`), **regenericizar** os arquivos que carregam valor específico do cliente (`next.config.ts` sem fallback hardcoded + `images.remotePatterns` com hostname wildcard `*.supabase.co`; `.env.local.example` sem URL de projeto; `docs/deploy.md`/`README.md` com checklist genérico citando migrations `0001`–`0009`; fixtures de teste com nome de exemplo em vez de "Tia Lu"), e validar (`npm install` limpo, typecheck ok, 111 testes passando, `npm run build` falha de propósito com `supabaseUrl is required` sem credenciais reais — prova de que não há fallback perigoso). Também levou o `docs/guia-de-conteudo.md` (é genérico, útil pra qualquer cliente — só trocadas as 4 menções a "Tia Lu"/"Luciana" por texto genérico). **Próxima vez que houver mudanças acumuladas:** repetir esse mesmo processo (não é incremental — recopia tudo e regenericiza de novo, já que o código de produção sempre tem os valores reais do cliente).
