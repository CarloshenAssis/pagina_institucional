...
5. ✅ **Aba "Aparência" no admin (2026-07-03), fechando a pendência acima.** `app/admin/(painel)/configuracoes/form.tsx` ganhou uma 5ª aba com 6 campos de cor (`<input type="color">` + hex texto, validados por `HEX` de `lib/content/theme.ts`, agora exportado junto com `THEME_COLOR_KEYS`). Novas actions `getTheme`/`saveTheme` em `actions.ts` (mesmo arquivo das outras). Bug pego e corrigido durante o teste: `saveTheme` validava `Object.entries(theme)` inteiro, incluindo `id`/`updated_at` que vêm no `select("*")` do Supabase (não só as 6 cores) — `"Cor inválida em id: 1"` 500 ao salvar; corrigido para validar só `THEME_COLOR_KEYS`. Testado ponta a ponta localmente contra o Supabase real (login → trocar cor secundária → salvar → refletir no `<style>` do portal → reverter), 0 erros de servidor, build+typecheck+107 testes verdes. Commit `5b51be9` na branch `claude/portal-publico-smoke-tests-hzhxp4` — **ainda não mergeado em `main`/deployado** (Carlos pediu para fechar todo o código antes de voltar a mexer em produção).
6. ✅ **Fix de `NEXT_PUBLIC_SITE_URL` (2026-07-03).** Achado ao reconferir produção: sem essa env var configurada na Vercel, `sitemap.xml`, `robots.txt` e o JSON-LD caíam no fallback `"http://localhost:3000"` hardcoded nesses arquivos — real em produção (`curl .../sitemap.xml` mostrava `<loc>http://localhost:3000/...</loc>`), quebraria indexação no Google. Corrigido com o mesmo padrão dos fallbacks já existentes no `next.config.ts` para as chaves do Supabase: `NEXT_PUBLIC_SITE_URL` cai em `https://pagina-institucional-chi.vercel.app` se não estiver setada no ambiente. Mesmo commit `5b51be9`, mesma branch — também não deployado ainda.
